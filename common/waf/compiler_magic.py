...
class check_binary_paths(Task.Task):
    def run(self):
        ...
        rpath = list(compiled_prog_paths)
        conf_ctx.env.append_value("RPATH", rpath)
        conf_ctx.bld.msg("Compiler RPATH", ":".join(rpath), color="WHITE")
...
def check_compiler_paths(self):
    ...
    ld_library_path = list(compiler_paths)
    self.env.append_value("LD_LIBRARY_PATH", ld_library_path)
    self.bld.msg("Compiler LD_LIBRARY_PATH", ":".join(ld_library_path), color="WHITE")
